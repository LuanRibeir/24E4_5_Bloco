class MinHeap:
    ...
    def buscar_elemento(self, elemento):
        # Busca um elemento na heap e retorna True se ele existir, False se não.
        return elemento in self.heap

    def remover_minimo(self):
        # Remove o menor elemento (raiz da heap) e reorganiza a heap.
        if not self.heap:
            return None  # Heap vazia
        
        print(f"Heap antes da remoção: {self.heap}")
        min_element = self.heap[0]
        self.heap[0] = self.heap[-1]
        self.heap.pop()
        self.heapify_down(0)
        print(f"Heap após a remoção: {self.heap}")
        return min_element
...
print("O elemento 7 está na heap?", heap.buscar_elemento(7))
